...
function send_sms($mobile, $message, $word = 0, $time = 0) {
	...
	if(!is_mobile($mobile)) return false;
	$params = array ();
    ...
    $params["TemplateCode"] = $message['TemplateCode'] ? $message['TemplateCode'] : "SMS_174986841";

    // fixme 可选: 设置模板参数, 假如模板中存在变量需要替换则为必填项
	$captcha = $message['MobileCode'] ? $message['MobileCode'] : random(4, '0123456789');
	...
	require_once DT_ROOT.'/api/SignatureHelper.php';
	...
	$code = $content->Code;
	DB::query("INSERT INTO ".DT_PRE."sms (mobile,message,word,editor,sendtime,ip,code) VALUES ('$mobile','$captcha','4','$_username','".DT_TIME."','".DT_IP."','$code')");
	
    return $content;
}

function send_sms_org($mobile, $message, $word = 0, $time = 0) {
	global $DT, $_username;
...
function get_maincat($catid, $moduleid, $level = -1) {
	...
	$cat = array();
	$sql = "SELECT catid,catname,child,style,linkurl,parentid,item,ranges,price,thumb,seo_keywords,seo_description FROM ".DT_PRE."category WHERE $condition ORDER BY listorder,catid ASC";
...
function checkcaptcha($captcha, $enable = 1, $return = false) {
	global $DT, $session;
	if($enable) {
		if($DT['captcha_cn']) {
			if(strlen($captcha) < 4) {
				return 'captcha_missed';
			}
		} else {
			if(!preg_match("/^[0-9a-z]{4,}$/i", $captcha)) {
				return 'captcha_missed';
