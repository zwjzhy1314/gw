...
<?php } else { ?>
<?php if($v['username']) { ?>
<a href="<?php echo $MOD['linkurl'];?><?php echo rewrite('search.php?username='.$v['username']);?>" class="b" target="_blank"><?php echo $v['passport'];?></a>
<?php } else { ?>
<span title="<?php echo hide_ip($v['ip']);?>">游客</span>
<?php } ?>
<?php } ?>
</td>
<td><?php echo $v['vote'];?></td>
<td align="left"><div class="vote_b"><div class="vote_s" style="width:<?php echo $v['precent'];?>;"> </div></div></td>
<td><?php echo $v['precent'];?></td>
</tr>
<?php } } ?>
</table>
<center><input type="button" value="返 回" onclick="Go('?itemid=<?php echo $itemid;?>');" class="btn-green"/></center>
<br/>
</div>
</div>
<?php } else { ?>
...
<?php if($items>1) { ?>
<div><a href="?itemid=<?php echo $itemid;?>&action=vote" onclick="return confirm('确定要举行投票吗？');" class="b" target="_top">举行投票</a><span class="f_gray"> - 不知道哪个回答最好，让网友投票来选出最佳答案;</span></div>
<?php } ?>
...
<?php } ?>
<div><a href="?itemid=<?php echo $itemid;?>&action=close" onclick="return confirm('确定要关闭这个问题吗？');" class="b" target="_top">关闭问题</a><span class="f_gray"> - 没有满意的回答 ，可以直接结束提问，关闭问题;</span></div>
</div>
</div>
</div>
<?php } ?>
...
<?php if($could_admin && $item['process'] == 1) { ?>
<a href="javascript:choose_answer(<?php echo $v['itemid'];?>);" class="b">采纳为答案</a>&nbsp;
<?php } ?>
<?php if($item['process'] == 2) { ?>
<?php if($could_admin) { ?>
<span onclick="if(confirm('确定要移除此投票选项吗？')) Dd('send').src='?itemid=<?php echo $itemid;?>&aid=<?php echo $v['itemid'];?>&action=vote_del';" class="c_p f_dblue"/>移除选项</span>&nbsp;
<?php } ?>
<?php if(!$could_vote || $could_admin) { ?>
<span onclick="Go('?itemid=<?php echo $itemid;?>&action=vote_show');" class="c_p f_dblue"/>投票结果</span>&nbsp;
<?php } ?>
<?php if($could_vote) { ?>
<span onclick="Dd('send').src='?itemid=<?php echo $itemid;?>&aid=<?php echo $v['itemid'];?>&action=vote_add';" class="c_p f_dblue"/>投一票</span>&nbsp;
<?php } ?>
<?php } ?>
</span>
...
<div style="padding-top:10px;"><a href="<?php echo $MODULE['2']['linkurl'];?><?php echo $DT['file_my'];?>?mid=<?php echo $moduleid;?>&job=answer&action=add&itemid=<?php echo $itemid;?>" target="_top" class="b" title="高级模式支持文字排版和图片上传，点击切换">高级模式</a> <span id="dcontent" class="f_red px12"></span></div>
</td>
</tr>
<tr>
...
function H() {
Dd('proxy_iframe').src = '<?php echo $MOD['linkurl'];?>ajax.php?action=proxy&itemid=2#'+Dd('destoon_answer').scrollHeight+'|<?php if($could_answer) { ?>1<?php } else { ?>0<?php } ?>';
}
