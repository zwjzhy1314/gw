...
switch($action) {
	...
	case 'sms':
		...
		if($submit) {
			(is_mobile($mobile) && preg_match("/^[0-9]{4}$/", $code) && isset($_SESSION['mobile_code']) && $_SESSION['mobile_code'] == md5($mobile.'|'.$code)) or message('验证失败');
	...
	case 'sendsms':
		...
		$mobilecode = random(4, '0123456789');
		...
		$message = array('MobileCode'=>$mobilecode,'TemplateCode'=>'SMS_174986841');
