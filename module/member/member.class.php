...
class member {
    ...
    function __construct() {
		...
		$this->table_member_child = DT_PRE.'member_child';
	...
	function addchild($member) {
		global $DT, $MOD, $L;
		$member_fields = array('nikename','gender', 'birthday','userid','scheduleids','recommendids');
        ...
        $member_sqlk = substr($member_sqlk, 1);
        $member_sqlv = substr($member_sqlv, 1);
        DB::query("INSERT INTO {$this->table_member_child} ($member_sqlk,ip,addtime)  VALUES ($member_sqlv,'".DT_IP."','".DT_TIME."')");
		$cid = DB::insert_id();
		if(!$cid) return 0;	
		return $cid;
	}
	
	function updatechild($member) {
		global $DT, $MOD, $L;
		$member_fields = array('nikename','gender', 'birthday','scheduleids','recommendids');
		$member_sql = '';
		foreach($member as $k=>$v) {
		...
		$id = $member['id'];
        DB::query("UPDATE {$this->table_member_child} SET $member_sql WHERE `id`=$id");
		return true;
	}
	
	function get_one_child($id) {
		global $DT, $MOD, $L;
		if(empty($id)) return false;
		$r = DB::get_one("SELECT * FROM {$this->table_member_child} WHERE `id`=$id");
		$r['addtime'] = timetodate($r['addtime'], 5);
		if($r['updatetime']) $r['updatetime'] = timetodate($r['updatetime'], 5);
		return $r;
	}
	
	function get_list_child($userid) {
		global $DT, $MOD, $L;
		if(empty($userid)) return false;
		$childs = array();
		$result = DB::query("SELECT * FROM {$this->table_member_child} WHERE `userid`=$userid");
